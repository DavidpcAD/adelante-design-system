...
export interface ScrollPickerProps {
  /** Si est├í abierto, muestra el bottom sheet */
  open: boolean;
  ...
  /** Texto del bot├│n confirmar. Default "Confirmar" */
  confirmLabel?: string;
}

/**
 * ScrollPicker ÔÇö bottom sheet con rueda scrolleable estilo iOS para elegir cantidades.
 *
 * Tres formas de cambiar el valor:
 *  - Scrollear el wheel (snap-to-row con momentum)
 *  - Tap en el pill grande ÔåÆ swap a input num├®rico
 *  - Combinaci├│n: input sincroniza al wheel y viceversa
 *
 * Se renderiza v├¡a portal a `document.body` para escapar del stacking context
            ...
            {contextLabel && (
              <header className="ds-sp__header">
                ...
                <motion.button
                  ...
                  aria-label="Cerrar"
                >
                  <Icon name="delete" size="lg" color="currentColor" />
                </motion.button>
              </header>
            )}
            ...
            <div className="ds-sp__value-row">
              {editing ? (
                ...
                <button
                  ...
                  onClick={startEdit}
                  aria-label={`Cantidad ${value}, toc├í para escribir`}
                >
                  <QuantitySelector value={value} state={variant} size="lg" />
                </button>
              )}
              <span className="ds-sp__value-hint">
                {editing ? "Enter para confirmar" : "Toc├í para escribir"}
              </span>
            </div>
